...
//TODO Kyle: "Importantly, for your program to properly work, it must accurately handle numbers."?

Two additions to `Number` are just references to the preexisting globals: `Number.parseInt(..)` and `Number.parseFloat(..)`.

### Static Properties

//TODO Kyle: Will you please include some text here to introduce the list that follows?  The first element following a heading should be a regular para.
