...
//TODO Kyle: Should it be `console.log()`? I didn't catch this in the previous book, but I don't think I've seen .. used as a placeholder in other books.  Is this a standard usage?  Same query for `log(..)`, `alert(..)`, etc. This query comes up in a few other places in the book, but I only queried on the first instance of each usage that has something in common (for example, in this case, the .. is in parens, while in the next query it's in quotes). I won't query again for other (..) instances, but you will see a couple other queries related to the usage of .. 
...
//TODO Kyle: Is "the value in `b`" correct or should it be "the value of `b`"?
...
//TODO Kyle: Should it be (`" ... "`) and (`' .. '`) (i.e., ... instead of ..?
...
//TODO Kyle: Should it be `if...else` (i.e., ... instead of ..)?
...
//TODO Kyle: Should it be `do...while` (i.e., ... instead of ..)?
...
//TODO Kyle: Should it be `while...true` (i.e., ... instead of ..?
...
The phone store employee probably doesn't carry around a calculator to figure out the taxes and final purchase amount for your phone. That's a task she needs to define once and reuse over and over again. Odds are, the company has a checkout register with those "functions" built in.
...
If you ask the phone store employee for a phone model that the store doesn't carry, she will not be able to sell you the phone you want. She only has access to the phones in her store's inventory. You'll have to try another store to see if you can find the phone you're looking for.
...
//TODO Kyle: Maybe set up the preceding para ("Because this is a JavaScript book...") as a note?
...
The next chapter will review many of the concepts from this chapter, but from a JavaScript-specific perspective, which will highlight most of the major topics that are addressed in deeper detail throughout the rest of the series.
